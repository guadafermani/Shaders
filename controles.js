...
var rotX=0, rotY=0, transZ=3, autorot=0, cosinerot=0;

var lastModel=null, lastShininess=null;
...
function InitWebGL()
{
	...
	// Extensión para utilizar derivadas
	gl.getExtension('OES_standard_derivatives');

	// Inicializar los shaders y buffers para renderizar	
	boxDrawer  = new BoxDrawer();
	meshDrawer = new MeshDrawer("phong");
...
window.onload = function() 
{
	...
	// Deshabilito control de efecto porque usamos phong
	document.getElementById('effect-exp').disabled = true;
	document.getElementById('cosine-rotate').disabled = true;
...
var cosineTimer;
function CosineTimer( param, show = true){
	if ( param.checked ) 
	{
		// Vamos rotando una cantiad constante cada 30 ms
		cosineTimer = setInterval( function() 
		{
				var v = document.getElementById('cosine-speed').value;
				cosinerot += 0.0005 * v;
				if ( cosinerot > 2*Math.PI ) cosinerot -= 2*Math.PI;

				// Reenderizamos
				meshDrawer.setTimer(cosinerot);
				if (show)
					DrawScene();
			}, 30
		);
		document.getElementById('cosine-speed').disabled = false;
	} 
	else 
	{
		clearInterval( cosineTimer );
		document.getElementById('cosine-speed').disabled = true;
	}
}

// Función que se llama cuando el usuario cambia la opción de shader seleccionada
function ReloadFragShader( param ){
	if ( param.checked){
		meshDrawer = new MeshDrawer(param.value);
	}
	LoadObj(document.getElementById('obj'), false);
	ShowTexture(document.getElementById('show-texture'), false);
	SwapYZ(document.getElementById('swap-yz'), false);
	SetShininess(document.getElementById('shininess-exp') , false);
	SetEffect(document.getElementById('effect-exp'), false);
	lightView.updateLightDir();
	LoadTexture(document.getElementById('texture'), true);
	shininessControlStatus(param.value)
	effectCheckStatus(param.value);
	effectSpeedCheckStatus(param.value)
}

// Habilitar el deslizador "Efecto" para todos los shaders excepto para el phong
function effectCheckStatus(shader) {
	if (shader == 'phong') {
		document.getElementById('effect-exp').disabled = true;
	} else {
		document.getElementById('effect-exp').disabled = false;
	}
}

// Habilitar el deslizador "Velocidad efecto" solo para el Cosine y el Stripe shader
function effectSpeedCheckStatus(shader){
	switch(shader){
		case "phong":
			disableCosineTimer();
			break;
		...
		case "pencil":
			...
			break;
		default:
		document.getElementById('cosine-rotate').disabled = false;
	}
}

function disableCosineTimer(){
	cosineTimerEnabler( false );
}
function cosineTimerEnabler( enable ){
	document.getElementById('cosine-rotate').disabled = !enable;
	document.getElementById('cosine-speed').disabled = !enable;
}
function enableCosineTimer(){
	cosineTimerEnabler( true );
}

// Habilitar o deshabilitar el control del deslizador "Brillo" según corresponda a cada shader
function shininessControlStatus(shader){
	switch(shader){
		case "phong":
			document.getElementById("shininess-exp").disabled = false;
			break;
		case "toon":
			document.getElementById("shininess-exp").disabled = true;
			break;
		case "cosine":
			document.getElementById("shininess-exp").disabled = false;
			break;
...
function ShowTexture( param, show = true )
{
...
function SwapYZ( param, show = true)
{
...
// Cargar archivo obj
function LoadObj( param, show = true )
{
...
// Habilitar que el usuario pueda seleccionar otros shaders además del phong
function enableShaders(){
	var elements = document.getElementsByName('shader-selection');
...
// Cargar textura
function LoadTexture( param, show = true)
{
	if ( param.files && param.files[0] )
	{
		var reader = new FileReader();
...
// Setear Intensidad
function SetShininess( param, show = true)
{
	var exp = param.value;
...
// Setear Efecto
function SetEffect( param, show = true)
{
	var exp = param.value;
	var s = exp / 10;
	document.getElementById('effect-value').innerText = s.toFixed( s < 10 ? 2 : 0 );
	meshDrawer.setEffect(s);
	if (show)
		DrawScene();
}